...
def test_get_datasets():
    ...
    datasets = repo.get_datasets(bucket=BUCKET, sensor_prefix='common_sensing/fiji/sentinel_2/')

    assert datasets == ['common_sensing/fiji/sentinel_2/S2A_MSIL2A_20151022T222102_T01KBU/',
...
def test_get_measurements():
    ...
    repo = repository.S3Repository(s3)
    measurements = repo.get_measurements(bucket=BUCKET, dataset_prefix='common_sensing/fiji/sentinel_2/'
                                                                       'S2A_MSIL2A_20151022T222102_T01KBU/')

    assert measurements == ['S2A_MSIL2A_20151022T222102_T01KBU_AOT_10m', 'S2A_MSIL2A_20151022T222102_T01KBU_B01_60m',
...
@mock_s3
def test_get_smallest_measurement_sample():
    s3 = S3(key=None, secret=None, s3_endpoint=None, region_name='us-east-1')
    initialise_cs_bucket(s3_resource=s3.s3_resource, bucket_name=BUCKET)

    repo = repository.S3Repository(s3)
    measurement = repo.get_smallest_measurement_sample(bucket=BUCKET,
                                                       dataset_prefix='common_sensing/fiji/sentinel_2/'
                                                                      'S2A_MSIL2A_20151022T222102_T01KBU/')
